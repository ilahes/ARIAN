...
|--------|------|--------|----------------------|
| **Raul** | Data Collection + Wildfire Modeling | 1, 4 | Fetches all raw data (Open-Meteo, FIRMS, ESA, WorldPop, MESE QURULUSU); builds ingestion pipeline & DuckDB schema; computes FWI & SPEI; trains wildfire classifier (`fire_occurred`) and regressor (`fire_count`) |
| **Aysu** | Weather Forecasting + API Backend | 2, 5 | Weather feature engineering (lags, rolling windows, sin/cos wind); trains HGBR forecast models per horizon; builds FastAPI app and `/weather` endpoint |
| **Ilaha** | EDA + Model Evaluation | 2, 4 | Data cleaning and distribution checks; wildfire model calibration curves, PR curves, threshold optimization; per-city performance breakdown |
| **Asif** | Feature Engineering + Climate Analysis + API Routes | 3, 4, 5 | Computes FWI features; spatial joins of forest inventory to city buffers; Mann-Kendall climate trend analysis; builds `/wildfire` and `/insights` API routes |
| **Nurana** | EDA + Frontend | 2, 5 | Cross-city EDA; horizon interpolation validation; dashboard UX; `/health` endpoint; end-to-end integration testing |
